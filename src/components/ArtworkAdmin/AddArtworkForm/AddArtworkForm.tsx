...
import {artworksApi} from "../../../store/artworks/artworks.api";
import {IArtworks} from "../../../models/models";

interface AddArtworkFormPropsType {
    handleClose: () => void;
}


const AddArtworkForm:FC<AddArtworkFormPropsType> = ({handleClose}) => {
    ...
    const [isAvailable, setIsAvailable] = useState<string>("Available");
    const [price, setPrice] = useState<string>("0");
    const [currency, setCurrency] = useState<string>("USD");
    const [createArtwork, {isError, isSuccess, error}] = artworksApi.useCreateArtworkMutation()
        ...
        <div className="flex justify-center my-14">
            <form className="" onSubmit={onSubmit}>
                ...
                <Button startIcon={<FileUploadIcon/>} color="secondary" sx={{mx: 2, minWidth: 163}} size="large"
                        variant="outlined" type="submit" >Add artwork</Button>
                {isError && <div className="m-4 p-2 text-center text-sm text-red-500 border">Возникла ошибка при загрузке на сервер!</div>}
            </form>
        </div>
    );
};
